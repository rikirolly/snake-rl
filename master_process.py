...
class MasterProcess():
    ...
    def train_agents(self):
        ...
        scores_received = threading.Event()
        t0 = time.time()
        def listenToAgent(id, scores):
            while True:
                msg = pipes[id].recv()
                if msg == "saved":
                    print("Master process (0) saved his weights.", flush=True)
                    for j in pipes:
                        if(j != 0):
                            pipes[j].send("load")
                else:
                    ...
                    with scores_lock:
                        scores[id] = score
                        batchs[id] = msg[1]
                    print("Process "+str(id)+" returns score "+str(score), flush=True)
                    scores_received.set()

        threads_listen = []
        print("Threads to start", flush=True)
        for id in pipes:
        ...
        while True:
            scores_received.wait()
            with scores_lock:
                if(len(scores) == variables.n_process):
                    scores_received.clear()
                    id_best = min(scores, key=scores.get)
                    mean_scores.append(np.mean(list(scores.values())))
                    print("End of iteration "+str(iter)+". Mean score sor far : "+str(np.mean(mean_scores)), flush=True)
                    iter += 1
                    file.write(str(np.mean(mean_scores))+"\n")
                    file.flush()
                    print("Time : "+str(time.time()-t0), flush=True)
                    print("\n", flush=True)
